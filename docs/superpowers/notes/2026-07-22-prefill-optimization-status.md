...
⚠️ **精度回归：根因已定位，修复已提交（commit `e7e975519`），待验证**：优化配置 GSM8K 0.765→0.363、MMLU 0.612→0.021。排查链：开关二分 R0-R5 锁定 FUSED_A1（R5 仅关它即恢复）；离线对拍无 -1 时逐位全等；构造 CUDA graph 补齐行（topk_ids=-1，`_mask_topk_ids_padded_region`）后 fused A1 复现 illegal memory access。**根因**：fused A1/A2 CUDA kernel 以 `uint32_t` 读 topk_ids，-1 回绕成 4294967295，`m_indptr[expert]` 越界读 ~17GB，衍生 scale 列成为任意写——serving 中落在 43GB 分配器池内静默踩踏相邻张量（离线小分配则直接 fault）。修复：A1/A2 kernel 路由循环加 `expert >= num_experts` 守卫（无符号比较覆盖 -1）+ expert 0 前缀 scale 列清零（两缓冲均 torch.empty 分配）；legacy Triton pack-scale kernel 补显式 `experts >= 0` mask（其此前的容忍是布局巧合的良性未定义行为）。教训：fused A1 是唯一无专属逐位测试的优化项，且此前全部 serving 验证为 output-len=1 纯 prefill，decode/语义正确性首次在精度测试中覆盖。
