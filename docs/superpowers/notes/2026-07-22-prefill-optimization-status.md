...
5. **免费档修复已提交（commit `a7e732638`）**：B1 chunk_o `zeros_like→empty_like`（隐藏 memset ~2.0GB/fwd）；B10 视图分支删 a/b `.contiguous()`（fused_gdn_gating 接受 stride）；B9 缓存 `norm.weight.repeat`；B13 `SGLANG_FLASHINFER_SM120_FP8_FUSED_A1` 默认翻 True。待服务器验证
6. 待排期（中置信，M/L 工作量）：D1-a（GDN gated norm 直出 fp8 喂 out_proj，估 -3.3ms，需解除 fp8_utils `assert input_scale is None`）；GDN1（conv1d epilogue 内做 l2norm+v 分流，估 -5.7ms）；M5（counting sort 替 torch.sort，估 -1.2ms，实测 sort 1.19ms）；B8（chunk_fwd A=zeros 改 empty+三角 mask）；D-ext 上半段 gather-A（-6.0ms，XL）——注意 T1 失败对 epilogue scatter-back 下半段是风险信号

当前累计：per-forward 241.4 → **≈226.4~226.8ms（-6.1%，ab 四轮同热态复核）**；端到端最近四轮 34542~34854 tok/s（相对基线 32659.7 约 **+5.8~6.7%**，最终入账需 300×3 中位数双边对比）。距 217ms 目标尚差 ~10ms：**剩余路径仅 D（tile 调优，修正后估 -2ms）与 D-ext（gather-aware GEMM + epilogue 融合，理论 -12ms，含 GEMM2 输出 268MB bf16 写+读往返的消除）**，达成 10% 需 D-ext 落地。生产环境变量清单（已验证）：`SGLANG_GDN_CHUNK_H_BV=64 _NUM_WARPS=4 _NUM_STAGES=2, SGLANG_GDN_WU_BK=128 _BV=128 _NUM_STAGES=3, SGLANG_FLASHINFER_SM120_FP8_FUSED_A1=1`；`SGLANG_GDN_QKV_VIEW=1` 默认保留。
