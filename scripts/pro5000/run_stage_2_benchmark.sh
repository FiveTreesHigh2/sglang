...
capture_after() {
  ...
  uv pip check --python "${PYTHON}" \
    >"${RUN_DIR}/pip-check-after.txt" 2>&1
  if [[ $? -ne 0 ]]; then
    AFTER_STATUS=1
  fi

  uv pip freeze --python "${PYTHON}" \
    >"${RUN_DIR}/packages-after.txt" 2>&1
  if [[ $? -ne 0 ]]; then
    AFTER_STATUS=1
  fi
  ...
  if [[ -f "${RUN_DIR}/packages-before.txt" && -f "${RUN_DIR}/packages-after.txt" ]]; then
    if ! cmp -s "${RUN_DIR}/packages-before.txt" "${RUN_DIR}/packages-after.txt"; then
      echo "ERROR: Python package set changed during Stage 2" >&2
      diff -u "${RUN_DIR}/packages-before.txt" \
        "${RUN_DIR}/packages-after.txt" \
...
finish() {
  ...
  echo "STAGE_2_STATUS=${BENCHMARK_STATUS}"
  echo "STAGE_2_WRAPPER_STATUS=${final_status}"
  if [[ -n "${RUN_DIR}" ]]; then
...
if [[ "$(uname -s)" != "Linux" || "$(uname -m)" != "x86_64" ]]; then
  echo "ERROR: Stage 2 requires Linux x86_64" >&2
  exit 1
fi
...
if [[ "$("${PYTHON}" -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")')" != "3.12" ]]; then
  echo "ERROR: Stage 2 requires the existing Python 3.12 Stage B venv" >&2
  exit 1
fi
if [[ -n "$(git -C "${REPO_ROOT}" status --porcelain)" ]]; then
  echo "ERROR: server checkout must be clean before Stage 2" >&2
  git -C "${REPO_ROOT}" status --short >&2
  exit 1
fi
if [[ -n "${FLASHINFER_DISABLE_JIT:-}" ]]; then
  echo "ERROR: unset FLASHINFER_DISABLE_JIT for Stage 2" >&2
  exit 1
fi
...
RUN_ID="stage-2-$(date -u +%Y%m%dT%H%M%SZ)-$(git -C "${REPO_ROOT}" rev-parse --short=12 HEAD)"
...
set +e
"${PYTHON}" "${REPO_ROOT}/scripts/pro5000/benchmark_flashinfer_sm120_fp8_runner.py" \
